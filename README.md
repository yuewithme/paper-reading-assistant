...
- PDF 后台导入、文件去重和 PaddleOCR PP-StructureV3 逐页解析；
- 第一页识别完成即可进入双栏阅读，后续页、译文和解读自动刷新；
- 无论 PDF 是否带文本层，都统一执行 OCR、版面分析和阅读顺序恢复；
- OCR 完成后自动生成逐段中文翻译并缓存；
- 原文、译文和 AI 解读统一支持安全 Markdown 与 KaTeX 公式渲染；
...
4. 第一页识别完成即可开始阅读；后续页面、译文和深度解读会在后台持续补全。

PP-StructureV3 会恢复多栏阅读顺序，并识别标题、正文、表格、图片说明和公式区域，同时保留页面号与区域坐标。它能显著改善复杂版面，但 OCR 与公式识别仍可能出错，尤其是低分辨率扫描、手写内容和非常规公式排版。

阅读区支持 `$...$`、`$$...$$`、`\(...\)` 和 `\[...\]` 四种常见 LaTeX 分隔符。OCR 文本入库前还会统一修复常见的 UTF-8 误解码、不可见零宽字符和 Unicode 规范化问题；无法被 OCR 正确识别的原始字形会保留为可读文本，不会导致页面渲染失败。
