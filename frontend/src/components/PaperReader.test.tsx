...
const paper: PaperDetail = {
  ...
  error_message: null,
  translations_completed: 2,
  analysis_group_count: 2,
  analysis_groups_completed: 2,
  ...
  paragraphs: [
    ...
    },
    {
      id: "paragraph-2",
      paragraph_index: 1,
      source_text: "Skipped source paragraph.",
      translated_text: "无需单独解读但应保留的原文。",
      page_number: 1,
      source_bbox_json: "{}",
    },
  ],
...
test("keeps skipped source paragraphs without rendering empty analysis cards", async () => {
  ...
  expect(screen.getByText("Skipped source paragraph.")).toBeInTheDocument();
  expect(screen.queryByText("GROUP 2")).not.toBeInTheDocument();
  expect(screen.queryByText("结构信息，无需单独解读。")).not.toBeInTheDocument();
  expect(
    container.querySelector(
      '[data-group-number="2"] .semantic-analysis--source-only',
    ),
  ).toBeInTheDocument();
});
